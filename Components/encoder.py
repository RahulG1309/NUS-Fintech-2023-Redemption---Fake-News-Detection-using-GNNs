...
class Encoder:
    ...
    def getUserTimeLine(self, userID):
        ...
        for i in range(len(response)):
            # Also remove URLs
            texts.append(re.sub(r"http\S+", "", response[i]._json["text"]))
    ...
    def profileEncoder(self, listOfUserIDs):
        ...
        try:
            if(len(listOfUserIDs) > 100):
                ...
                for i in range(100, len(listOfUserIDs), 100):
                    response += self.api.lookup_users(
                        user_id=listOfUserIDs[i-100:i])
        ...
        for profile in user_dict.values():
            ...
            user_date = datetime.strptime(
                profile['created_at'], '%a %b %d %H:%M:%S +0000 %Y')
            month_diff = (user_date.year - est_date.year) * \
                12 + user_date.month - est_date.month
            vector += [month_diff]

            # 9) Number of words in the description, 10) Number of words in the screen name
            vector += [len(profile['name'].split()),
                       len(profile['description'].split())]
    ...
    def getInputToModel(self, graph, article):
        ...
        # filter out the root (dummy node)
        userIds = [str(node) for node in G.nodes if node > 0]
