...
class Graph:
    ...
    def get_graph_of_tweet(self, tweet_id, nodes={}, retweet_count=-1):
        ...
        if retweet_count != 0:
            ...
            q, u = self.get_all_quote_tweets(
                tweet_id)  # getting al quote tweets
        ...
        for i in range(0, len(d) - 1):
            if d[i].id not in self.nodes:
                ...
                if following_count > 1000:
                    ...
                    print("Following count for {} : {}".format(
                        d[i].username, len(f)))
                    self.nodes[d[i].id] = f
                else:
                    ...
                    if limit > 0:
                        ...
                        try:
                            ...
                            f = []
                            self.get_following_manually(
                                arr=f, user_name=d[i].username)
                            print(
        ...
        # adding edge between earliest and cascade node
        G.add_edge(uid, d[-1].id)
        ...
        for i in range(0, len(d) - 1):
            if d[i].id not in self.nodes:
                ...
                if following_count < 2000:
                    f = []
                    self.get_following_manually(arr=f, user_name=d[i].username)
                    print("Following acount for {} : {}".format(
                        d[i].username, len(f)))
                    self.nodes[d[i].id] = f
                else:
                    limit = self.api.rate_limit_status()["resources"]["friends"][
    ...
    def get_following_manually(self, arr, user_name):
        ...
        def runProgram(option, header_option, user_id, count, arr):
            ...
            while True:
                ...
                firstCursor = infoExtractor(
                    json.loads(payload), 0, header_option, arr)
                if firstCursor == "":
                    break
                else:
